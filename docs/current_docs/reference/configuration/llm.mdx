...
Use your Claude Code Max subscription instead of API keys for zero per-token costs and higher rate limits.
...
2. Authenticate (one-time):
   ...
   The CLI will open a browser for authentication and store credentials in `~/.claude/.credentials.json`.

3. Use in any Dagger module:
...
**Model Options:**
- `claude-code` or `claude-code-sonnet` - Claude Sonnet 4.5 (default, best balance)
- `claude-code-opus` - Claude Opus 4.1 (most capable)
- `claude-code-haiku` - Claude Haiku 4.5 (fastest)

**How it works:**

When building Dagger from source, the engine includes Node.js, npm, and the Claude CLI. Your host credentials (`~/.claude/.credentials.json`) are automatically mounted read-only into the engine container, enabling seamless authentication.
...
review, err := dag.LLM().
    ...
    WithPrompt("Review this pull request...").
    LastReply(ctx)
```

:::note
Tool calling is disabled in this mode for simple text completion. Requires Dagger engine built from source.
:::

:::tip Troubleshooting
If you get "claude CLI not found", ensure you're using a locally built Dagger engine (`./hack/build`). If authentication fails, re-run `claude` to re-authenticate.
:::
